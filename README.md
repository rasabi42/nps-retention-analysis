...
### Stage 2 — SQL
- Ran sanity checks (row counts, duplicate emails, year breakdown) on the pre-combined survey data and the behavioral export before joining
- Deduplicated the behavioral export using a CTE with `ROW_NUMBER() OVER (PARTITION BY LOWER(TRIM(email)) ORDER BY first_app_open_d DESC)`, keeping only the most recent record per user where multiple entries existed
 Joined the deduplicated behavioral table to survey data via `LEFT JOIN ON LOWER(TRIM(s.email)) = LOWER(TRIM(b.email))` to handle casing and whitespace inconsistencies
